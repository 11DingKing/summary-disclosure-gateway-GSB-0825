...
export function requireBody(body: Record<string, unknown>, maxLength = 200_000): string {
...
export function parseDateField(body: Record<string, unknown>, field: string): Date {
  const value = body[field];
  if (typeof value !== 'string') {
    throw validationError(`"${field}" must be an ISO-8601 date-time string`, { field });
  }
...
export function parseStringArray(
  ...
  for (const item of value) {
    if (typeof item !== 'string' || item.trim().length === 0) {
      throw validationError(`"${field}" must contain only non-empty strings`, { field });
    }
    out.push(item);
  }
  return out;
...
export function parsePageRange(body: Record<string, unknown>): {
  pageStart: number;
  pageEnd: number;
} {
  ...
  if (pageStart > pageEnd) {
    throw validationError('"pageStart" must be less than or equal to "pageEnd"', {
      fields: ['pageStart', 'pageEnd'],
    });
  }
...
export function parseCitationSpans(
  ...
  if (!Array.isArray(raw) || raw.length === 0) {
    throw validationError('"citations" must be a non-empty array of CitationSpan objects', {
      field: 'citations',
    });
  }
  if (raw.length > 1000) {
  ...
  return raw.map((item, ordinal) => {
    if (typeof item !== 'object' || item === null || Array.isArray(item)) {
      throw validationError(`citations[${ordinal}] must be an object`, { ordinal });
    }
    const citation = item as Record<string, unknown>;
    const sourceBlockId = citation.sourceBlockId;
    if (typeof sourceBlockId !== 'string' || sourceBlockId.trim().length === 0) {
      throw validationError(`citations[${ordinal}].sourceBlockId must be a non-empty string`, {
        ordinal,
        field: 'sourceBlockId',
      });
    }
