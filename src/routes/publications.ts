...
import { AppError, notFound, policyViolation, validationError } from '../lib/errors.js';
...
export interface PublicationsRouteOptions {
  db: PrismaClient;
}

const DEFAULT_SCOPE = 'default';

const publicationsRoutes: FastifyPluginAsync<PublicationsRouteOptions> = async (
  app,
  options,
) => {
  const db = options.db;
  ...
  app.post('/publications', async (request, reply) => {
    ...
    const outcome = await withIdempotency(
      ...
      body,
      async () => {
        const result = await withBusyRetry(() => publishOnce(db, body));
        return result;
      },
    );
  ...
  app.get('/publications', async (request) => {
    ...
    if (query.scope !== undefined) {
      if (typeof query.scope !== 'string' || query.scope.trim().length === 0) {
        throw validationError('"scope" must be a non-empty string', { field: 'scope' });
      }
  ...
  app.get('/publications/:id', async (request, reply) => {
    ...
    return reply.send(serializePublication(publication, parseSnapshot(publication.snapshot)));
  });
};
...
async function publishOnce(
  ...
  const scope =
    typeof body.scope === 'string' && body.scope.trim().length > 0
      ...
      : DEFAULT_SCOPE;
  if (scope.length > 100) {
    throw validationError('"scope" must not exceed 100 characters', { field: 'scope' });
  }
  ...
  const submissionIds = [...new Set(parseStringArray(body.submissionIds, 'submissionIds'))];
  ...
  return db.$transaction(
    async (tx) => {
      ...
      const citedSources: SubmissionWithCitations[] = citedSourceIds.length
      ...
      const allSubmissions: SubmissionWithCitations[] = [...selected, ...citedSources];
      ...
      const serializedSubmissions: Record<string, unknown>[] = [];
