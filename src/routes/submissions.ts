...
const submissionsRoutes = async (
  ...
  app.get("/submissions", async (request) => {
    ...
    if (query.kind !== undefined) {
      if (!isKind(query.kind)) {
        throw validationError(
          '"kind" must be one of the four submission kinds',
          { field: "kind" },
        );
      }
    ...
    if (query.docKey !== undefined) {
      if (
        typeof query.docKey !== "string" ||
        query.docKey.trim().length === 0
      ) {
        throw validationError('"docKey" must be a non-empty string', {
          field: "docKey",
        });
      }
    ...
    const items = await Promise.all(
      page.map((row) => serializeSubmission(db, row)),
    );
  ...
  app.patch("/submissions/:id", async (request, reply) => {
    ...
    // Only SOURCE_EXCERPT carries a version and is revisable. Reader notes,
    // editor summaries and AI summaries are immutable after creation.
...
/**
 * Create a submission of one of the four kinds, enforcing per-kind required
 * fields. SOURCE_EXCERPT must declare version + page range (and gets a
 * server-computed contentHash). AI_SUMMARY must declare model metadata,
 * generatedAt, input source block ids and per-claim citations — all of which
 * are validated against real, existing SOURCE_EXCERPT blocks.
 */
async function createSubmission(
  db: PrismaClient,
  body: Record<string, unknown>,
): Promise<SubmissionWithCitations> {
  ...
  if (kind === "SOURCE_EXCERPT") {
    ...
    data.contentHash = sourceContentHash({
      version,
      sourceRevision,
      pageStart,
      pageEnd,
  ...
  if (kind === "AI_SUMMARY") {
    const modelProvider = requireNonEmptyString(
      body,
      "modelProvider",
      200,
    ).trim();
    ...
    const inputBlockIds = [
      ...new Set(parseStringArray(body.inputBlockIds, "inputBlockIds")),
    ];
    if (inputBlockIds.length === 0) {
      throw validationError(
        'AI_SUMMARY requires a non-empty "inputBlockIds" list',
        { field: "inputBlockIds" },
      );
    }
    ...
    const citationInputs = parseCitationSpans(
      body.citations,
      outputLength,
      sourceLength,
    );
